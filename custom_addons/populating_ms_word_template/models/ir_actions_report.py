...
class IrActionsReport(models.Model):
    ...
    populating_ms_word_template = fields.Boolean(
        "Populating MSWord Template", default=False
    )
    type_export = fields.Selection(
        [("pdf", "PDF"), ("docx", "Docx")], string="Export", default="docx"
    )
    ...
    def export_doc_by_template(
        self,
        file_template_data=None,
        suffix=".docx",
        file_name_export="export1",
        datas={},
    ):
        simple_merge = {}
        populating_tables = {}
        file_template = self._convert_binary_to_doc(
            file_template_data=file_template_data, suffix=".docx"
        )
                    ...
                    for data in data_array:
                        ...
                        if data_key.id == int(childs[1]):
                            if childs[2] != "label":
                                ...
                                if (
                                    data._name == "tests"
                                    and data.parameter_characteristic_id.unit_id.id
                                    != False
                                ):
                                    unit = data.parameter_characteristic_id.unit_id.name
                                    value = value + " " + unit
                                if isinstance(data, datetime):
                                    value = self._convert_datetime_usertz_to_utctz(
                                        value
                                    )
                                elif isinstance(value, date):
                                    value = value.strftime(
                                ...
                                if data._name == "tests":
                                    values = data.parameter_id.get_field_translations(
                                        "name"
                                    )
                                    for v in values[0]:
                                        if len(childs) < 4:
                    ...
                    if tmp_logic == "sum":
                        ...
                        for child in childs:
                            if child == "func()":
                                ...
                                data = getattr(data, child)
                        simple_merge[field] = str(data) if data else ""
                        # Much. Added for not showing False
    ...
    def render_doc_doc(self, res_ids=None, data=None):
        ...
        docx = self.export_doc_by_template(
            datas=res_ids[0], file_name_export=self.print_report_name, suffix=suffix
        )
    ...
    def num2word(self, amount):
        ...
        amount_words = tools.ustr("{amt_value} {amt_word}").format(
            amt_value=_num2words(integer_value, lang=lang_code), amt_word=""
        )
        return amount_words
    ...
    def _render_qweb_pdf(self, report_ref, res_ids=None, data=None):
        if self.populating_ms_word_template != True:
            result = super(IrActionsReport, self)._render_qweb_pdf(
    ...
    def _match_and_replace(self, block, record, fields):
        ...
        if matched_field and hasattr(record, field[0]):
            ...
            if len(field) == 3:
                block.add_run().add_picture(
                    tempfile_png.name, width=Inches(int(field[1]))
                )
            elif len(field) == 4:
                if field[2] == "cm":
                ...
                for data in data_ids:
                    if data.id == int(childs[0]):
                        if childs[1] == "line_fix":
                            ...
                            if childs[7] == "width":
                                block.add_run().add_picture(
                                    tempfile_png.name, width=Inches(int(childs[4]))
                                )
                            elif childs[8] == "width":
                                if childs[7] == "cm":
                                    ...
                                    width = int(field[5])
                                block.add_run().add_picture(
                                    tempfile_png.name, width=Inches(width)
                                )
                            else:
                                block.add_run().add_picture(tempfile_png.name)
                            break
